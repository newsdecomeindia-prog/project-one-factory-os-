...
router.get('/', requireAuth, requirePermission('productionreceipt:read'), async (req: Request, res: Response) => {
  try {
...
router.post('/', requireAuth, requirePermission('productionreceipt:create'), async (req: Request, res: Response) => {
  try {
    ...
    const exec = await prisma.productionExecution.findUnique({
    ...
    const existingReceipt = await prisma.productionReceipt.findFirst({
      where: { executionId },
    });
    if (existingReceipt) {
