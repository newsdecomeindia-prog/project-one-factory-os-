...
router.get('/', auth_1.requireAuth, (0, auth_1.requirePermission)('ipqc:read'), async (req, res) => {
    try {
        const user = req.user;
        ...
        const inspections = await prisma_1.prisma.inProcessQaInspection.findMany({
            where,
            include: {
...
router.post('/', auth_1.requireAuth, (0, auth_1.requirePermission)('ipqc:create'), async (req, res) => {
    try {
        const user = req.user;
        ...
        let woRecord = null;
        if (workOrderId) {
            woRecord = await prisma_1.prisma.workOrder.findUnique({ where: { id: workOrderId } });
            if (!woRecord) {
        ...
        const matRecord = await prisma_1.prisma.material.findUnique({ where: { id: materialId } });
        if (!matRecord) {
