...
router.get('/', auth_1.requireAuth, (0, auth_1.requirePermission)('stocktransfer:read'), async (req, res) => {
    try {
        const user = req.user;
        ...
        const transfers = await prisma_1.prisma.inventoryTransferOrder.findMany({
            where,
            include: {
...
router.post('/', auth_1.requireAuth, (0, auth_1.requirePermission)('stocktransfer:create'), async (req, res) => {
    try {
        const user = req.user;
        ...
        const sourcePlant = await prisma_1.prisma.plant.findUnique({ where: { id: String(sourcePlantId) } });
        if (!sourcePlant) {
...
router.post('/:id/approve', auth_1.requireAuth, (0, auth_1.requirePermission)('stocktransfer:approve'), async (req, res) => {
    try {
        const user = req.user;
        const transferId = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
        const transfer = await prisma_1.prisma.inventoryTransferOrder.findUnique({ where: { id: transferId } });
        if (!transfer) {
        ...
        const updated = await prisma_1.prisma.inventoryTransferOrder.update({
            where: { id: transferId },
            data: {
                status: 'APPROVED',
...
router.post('/:id/issue', auth_1.requireAuth, (0, auth_1.requirePermission)('stocktransfer:issue'), async (req, res) => {
    try {
        const user = req.user;
        const transferId = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
        const transfer = await prisma_1.prisma.inventoryTransferOrder.findUnique({ where: { id: transferId } });
...
router.post('/:id/receive', auth_1.requireAuth, (0, auth_1.requirePermission)('stocktransfer:receive'), async (req, res) => {
    try {
        const user = req.user;
        ...
        const atomicUpdate = await prisma_1.prisma.inventoryTransferOrder.updateMany({
            where: {
                id: recTransferId,
                status: 'IN_TRANSIT',
            },
        ...
        if (atomicUpdate.count === 0) {
            const current = await prisma_1.prisma.inventoryTransferOrder.findUnique({ where: { id: recTransferId } });
            if (!current) {
                return res.status(404).json({ success: false, error: 'Transfer order not found' });
            }
            return res.status(400).json({ success: false, error: `Transfer order cannot be received in status ${current.status}` });
        }
        const transfer = await prisma_1.prisma.inventoryTransferOrder.findUnique({ where: { id: recTransferId } });
        if (!transfer) {
            return res.status(404).json({ success: false, error: 'Transfer order not found' });
        }
        if (!user.isSuperAdmin && transfer.companyId !== user.companyId) {
...
router.post('/:id/cancel', auth_1.requireAuth, (0, auth_1.requirePermission)('stocktransfer:create'), async (req, res) => {
    try {
        const user = req.user;
        const transferId = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
